...
{
    /// <summary>
    /// Class to blast particles when the slime ghost hits the ball
    /// </summary>
    public class ExplosionEngine
    {
        ...
        public ExplosionEngine(Texture2D texture, Vector2 location, Color color)
        {
            Emit = location;
        ...
        private ExplosionParticle Generate()
        {
            Texture2D texture = spriteTexture;
            Vector2 position = Emit;

            float speed = 18f * (1f - 1 / (float)random.Next(1, 10));
            Vector2 velocity = new Vector2(random.Next(0, (int)speed) * 2, random.Next(0, (int)speed) * 2);
            float angle = 0;
            ...
            return new ExplosionParticle(texture, position, velocity, angle, angularVelocity, color, size, time);
        }
